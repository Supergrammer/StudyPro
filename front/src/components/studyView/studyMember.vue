    ...
    <v-list>
      ...
      <modal :open-modal="modal" :close="modalClose">
        <template v-slot:text>
          ...
          <p v-if="modalType == 'greeting'" class="mt-5 pt-2" style="white-space:pre;border-top: 1px solid #262F33;">{{selectedUser.comment}}</p>
        </template>
        <template v-slot:btn>
...
<v-card elevation="0">
  <v-card-title >"{{this.studyInfo.name}}" 스터디에 지금 가입하세요!</v-card-title>

  <v-card-actions>
        <v-row justify="center">
         
          <v-btn class="green" dark @click="modalOpen" >가입하기</v-btn>
        </v-row>
      </v-card-actions>
